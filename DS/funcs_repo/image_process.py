...
# Коефіцієнти функції для знаходження контурів окремих символів у функції find_contours
# contours, hierarchy = cv2.findContours(image, mode, method, contours=None, hierarchy=None, offset=None)
# MODE_COEFFICIENT = 'RETR_TREE'
# METHOD_COEFFICIENT = 'CHAIN_APPROX_SIMPLE'
...
def plate_recognize(photo):
    ...
    output_img, plate = detect_plate(img, text=current_datetime_str)
    ...
    char = segment_characters(plate)# Виявлення символів номерного знаку
    recognized_symbols = show_results(char)# Розпізнавання символів номерного знаку
